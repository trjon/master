...
var request = {};
var response = {};
var statusCode= context.getVariable("ep.buildingCreditResponse.status.code");
...
if(logType=="DEBUG"){
response.content = context.getVariable("ep.buildingCreditResponse.content");
}

rootElement.timeStamp=getFormattedDate();
rootElement.timeTaken = context.getVariable("apigee.metrics.policy.getCartPricePreview_cartPriceForBuildingCredit.timeTaken");
rootElement.url=context.getVariable("servicecallout.requesturi");
rootElement.verb = context.getVariable("cortexRequest.verb");
rootElement.request = request;
rootElement.response=response;
context.setVariable("buildingCreditCallLog", JSON.stringify(rootElement));
