...
var payNowPrice = responseObj._paynowprice;
var monthlyPrice = responseObj._monthlyprice;
...
var cartPricing = createPricing(payNowPrice,monthlyPrice,order,refundabledeposit, monthlyDiscounts);
...
function createPricing(payNowPrice,monthlyPrice,order,refundabledeposit, monthlyDiscounts) {		
	var pricing = {};
	    //Create Options
    var options = {};
	var discounts = [];
	...
	if (!isNullOrEmpty(payNowPrice) && !isNullOrEmpty(payNowPrice[0].cost)) {
		options.payNowAmount = payNowPrice[0].cost[0];
		subtotal.payNowPrice = payNowPrice[0].cost[0];
		total.payNowPrice = payNowPrice[0].cost[0];
	}
		if (!isNullOrEmpty(monthlyPrice) && !isNullOrEmpty(monthlyPrice[0].cost)) {
		options.monthlyPrice = monthlyPrice[0].cost[0];
		subtotal.monthlyPrice = monthlyPrice[0].cost[0];
		total.monthlyPrice = monthlyPrice[0].cost[0];
	}
...
function createsimCard(simCard, simCardSelected) {
	...
	for (var i=0; i<simCardDetails.length; i++) {
		...
		}else if (simCardDetail.name == 'TI' && !isNullOrEmpty(simCardDetail.value)) {
			simCard.taxInclusive = simCardDetail.value;
