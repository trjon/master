...
var request = {};
var response = {};
var statusCode= context.getVariable("raptorGetProfileAddressesResponse.status.code");
...
if(logType=="DEBUG"){
	response.content = context.getVariable("raptorGetProfileAddressesResponse.content");
}

rootElement.timeStamp=getFormattedDate();
rootElement.timeTaken = context.getVariable("apigee.metrics.policy.Callout.GetProfileAddresses.timeTaken");
rootElement.url=context.getVariable("servicecallout.requesturi");
rootElement.verb = context.getVariable("serviceCalloutGetProfileAddressesResponse.verb");
rootElement.request = request;
rootElement.response=response;

context.setVariable("getProfileAddressesServiceCallout", JSON.stringify(rootElement));
